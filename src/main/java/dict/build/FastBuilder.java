...
public class FastBuilder {
    ...
    public List<String> readExistingWordFile(String... existingWordFiles) {
        List<String> wordList = new ArrayList<>();
        for (String existingWordFile : existingWordFiles) {
            File file = new File(existingWordFile);
            if (file.exists()) {
                readFileForLine(file, wordList);
            }
        }
        return wordList;
    }

    private void readFileForLine(File file, List<String> wordList) {
        try {
    ...
    public void extractWords(String freqFile, String entropyFile) {
        extractWords(freqFile, entropyFile, null, null);
    }

    public void extractWords(String freqFile, String entropyFile, String natureType, List<String> existingWordList) {
        ...
        try (BufferedReader fr = Files.newReader(ffile, Charsets.UTF_8);
            ...
            while (null != (line = er.readLine())) {
                ...
                if (pmi < 1 || e < 2 || pp < 0.1)
                    continue;
                if (existingWordList != null && existingWordList.size() > 0 && existingWordList.contains(w)) {
                    continue;
                }
                if (null == natureType) {
                    ww.write(w + "\t" + "nz" + "\t" + f + "\t" + pmi + "\t" + e + "\t" + pp + "\n");
                } else {
                    ww.write(w + "\t" + "nz" + "\t" + f + "\n");
                }
